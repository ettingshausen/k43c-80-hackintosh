 ...
 *
 * Disassembly of iASL6x0kfH.aml, Mon Aug 10 23:40:03 2020
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0001CEE8 (118504)
 *     Revision         0x02
 *     Checksum         0x51
 *     OEM ID           "LENOVO"
 *     OEM Table ID     "CB-01   "
...
DefinitionBlock ("", "DSDT", 2, "LENOVO", "CB-01   ", 0x00000001)
{
    ...
    Name (SS3, One)
    Name (SS4, One)
    OperationRegion (GNVS, SystemMemory, 0x7FF47358, 0x0744)
    ...
    OperationRegion (OGNS, SystemMemory, 0x7FF47FD8, 0x00000010)
        ...
        Device (XHC)
        {
            ...
            Device (RHUB)
            {
                ...
                Device (HS04)
                {
                    ...
                    Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
                    {
                        If (LEqual (GGIV (0x02000016), Zero))
                        {
                ...
                Device (SS06)
                {
                    ...
                    Method (_PLD, 0, Serialized)  // _PLD: Physical Location of Device
                    {
                        Name (PLDP, Package (0x01)
                        {
                            ...
                            {
                                /* 0000 */  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                /* 0008 */  0x00, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 
                            }
                        })
                        Return (PLDP)
                    }
            ...
            Method (_CRS, 0, Serialized)  // _CRS: Current Resource Settings
            {
                ...
                Name (GBUF, ResourceTemplate ()
                {
                    GpioInt (Level, ActiveLow, ExclusiveAndWake, PullDefault, 0x0000,
                        "\\_SB.PCI0.GPI0", 0x00, ResourceConsumer, _Y20,
                        )
                        {   // Pin list
                            0x0000
                        }
                ...
                CreateDWordField (BBUF, \_SB.PCI0.SPI1.FPNT._CRS._Y1E._SPE, SPEX)  // _SPE: Speed
                CreateByteField (BBUF, \_SB.PCI0.SPI1.FPNT._CRS._Y1E._PHA, PHAX)  // _PHA: Clock Phase
                CreateWordField (BBUF, 0x3B, SPIN)
                CreateWordField (GBUF, 0x17, GPIN)
                CreateDWordField (IBUF, \_SB.PCI0.SPI1.FPNT._CRS._Y1F._INT, IPIN)  // _INT: Interrupts
                CreateBitField (IBUF, \_SB.PCI0.SPI1.FPNT._CRS._Y1F._LL, ILVL)  // _LL_: Low Level
                CreateBitField (IBUF, \_SB.PCI0.SPI1.FPNT._CRS._Y1F._HE, ITRG)  // _HE_: High-Edge
                CreateField (GBUF, \_SB.PCI0.SPI1.FPNT._CRS._Y20._POL, 0x02, GLVL)  // _POL: Polarity
                CreateBitField (GBUF, \_SB.PCI0.SPI1.FPNT._CRS._Y20._MOD, GTRG)  // _MOD: Mode
                CreateBitField (BBUF, \_SB.PCI0.SPI1.FPNT._CRS._Y1E._DPL, SCSP)  // _DPL: Device Selection Polarity
        ...
        Device (LDRC)
        {
            ...
            Name (_CRS, ResourceTemplate ()  // _CRS: Current Resource Settings
            {
                ...
                IO (Decode16,
                    ...
                    0x0078,             // Range Maximum
                    0x01,               // Alignment
                    0x01,               // Length
                    )
                IO (Decode16,
                    0x007C,             // Range Minimum
                    0x007C,             // Range Maximum
                    0x01,               // Alignment
                    0x01,               // Length
                    )
                IO (Decode16,
    ...
    Method (_WAK, 1, Serialized)  // _WAK: Wake
    {
        ...
        If (LOr (LEqual (Arg0, 0x03), LEqual (Arg0, 0x04)))
        {
            ...
            If (LEqual (OSYS, 0x07D9))
            {
                ...
                {
                    \_SB.SGOV (0x02040005, One)
                }
                Else
                {
                    \_SB.SGOV (0x02040005, Zero)
                }
            }
    ...
    OperationRegion (MDBG, SystemMemory, 0x7FF31018, 0x00001004)
    ...
    Scope (\)
    {
        OperationRegion (COMP, SystemMemory, 0x7FF57018, 0x0200)
        ...
        Scope (_SB.PCI0.I2C1)
        {
            Device (TPAD)
            {
                ...
                CreateWordField (SBFB, \_SB.PCI0.I2C1.TPAD._Y2A._ADR, ADR0)  // _ADR: Address
                Method (_HID, 0, Serialized)  // _HID: Hardware ID
                {
                    If (Not (CondRefOf (TPDF)))
                    {
        ...
        Scope (_SB.PCI0.I2C1)
        {
            Device (TPNL)
            {
                ...
                CreateWordField (SBFB, \_SB.PCI0.I2C1.TPNL._Y2B._ADR, ADR0)  // _ADR: Address
                Method (_HID, 0, Serialized)  // _HID: Hardware ID
                {
    ...
    Scope (_SB.PCI0.LPCB)
    {
        ...
        Device (EC0)
        {
            ...
            {
                Store (Buffer (0x08){}, Local0)
                Store (0xB1, Index (Local0, Zero))
                Store (Arg0, Index (Local0, One))
                Store (Arg1, Index (Local0, 0x02))
                ERCD (Local0)
            }
            ...
            Method (ERCD, 1, Serialized)
            {
                ...
                Name (ESRC, 0x64)
                Store (One, ECTB)
                Store (DerefOf (Index (Arg0, Zero)), ECMD)
                Store (DerefOf (Index (Arg0, One)), EDT1)
                Store (DerefOf (Index (Arg0, 0x02)), EDT2)
                Store (One, ECTE)
                While (LAnd (LNotEqual (ESRC, Zero), LNotEqual (ECTE, Zero)))
                {
                    Sleep (0x0A)
                    Subtract (ESRC, One, ESRC)
                }
        ...
        Scope (EC0)
        {
            ...
            {
                Store (Acquire (BAMT, 0x0064), Local0)
                Store (Arg1, SMAD)
                Store (Arg2, SMCM)
                Store (Zero, SMST)
                Store (Arg0, SMPR)
            ...
            Method (GBWD, 2, NotSerialized)
            {
                ...
                {
                    Store (ECSM (0x09, Arg0, Arg1), Local1)
                    If (Local1)
                    {
                        Break
                    }
            ...
            Method (GBMN, 1, NotSerialized)
            {
                ...
                If (ECOK ())
                {
                    Store (ECSM (0x0B, Arg0, 0x21), Local0)
                    If (Local0)
                    {
                        Store (SMD0, BFWB)
                        Store (FB70, CMMA)
            ...
            Method (GUBS, 1, NotSerialized)
            {
                ...
                If (ECOK ())
                {
                    Store (ToDecimalString (BSN0), BRSN)
                }
                Else
                {
                    Store ("41167", BRSN)
            ...
            Method (GUBI, 1, NotSerialized)
            {
                ...
                If (ECOK ())
                {
                    Store (ECSM (0x0B, Arg0, 0x20), Local0)
                    If (Local0)
                    {
                        Store (SMD0, BFWB)
                        Store (FBB0, BRMN)
        ...
        Device (BAT1)
        {
            ...
            Method (BIFX, 1, NotSerialized)
            {
                ...
                Store (Zero, ^^EC0.SMBN)
                Store (^^EC0.GBMN (0x16), BMLT)
                Store (BMLT, Index (STAX, 0x10))
                Store (^^EC0.GUBS (0x16), Index (STAX, 0x11))
                Store (^^EC0.GUBI (0x16), Index (STAX, 0x13))
                If (ECOK ())
                {
                    Store (^^EC0.ORRF, BFFW)
        ...
        Device (BAT2)
        {
            ...
            Method (BIFX, 1, NotSerialized)
            {
                ...
                Store (Zero, ^^EC0.SMBN)
                Store (^^EC0.GBMN (0x36), ^^BAT1.BMLT)
                Store (^^BAT1.BMLT, Index (STAX, 0x10))
                Store (^^EC0.GUBS (0x36), Index (STAX, 0x11))
                Store (^^EC0.GUBI (0x36), Index (STAX, 0x13))
                If (ECOK ())
                {
                    Store (^^EC0.ORRF, BFFW)
                    If (LEqual (^^EC0.BAM1, Zero))
                    {
                        Store (0x0A, BASC)
                    }

                    Store (^^EC0.BAM1, Index (STAX, One))
                    If (LEqual (BML1, "11111111"))
                    {
                        Store (^^BAT1.BMLT, BML1)
                    }
                    Else
                    {
                    }

                    Store (Multiply (^^EC0.BDC1, BASC), Index (STAX, 0x02))
                    Store (^^EC0.BDV1, Index (STAX, 0x05))
                    If (LEqual (BFFW, One))
                    {
            ...
            Method (_BST, 0, NotSerialized)  // _BST: Battery Status
            {
                ...
                If (ECOK ())
                {
                    Store (^^EC0.BST1, Index (PBST, Zero))
                    If (ShiftRight (^^EC0.BAC1, 0x0F))
                    {
                        Add (And (Not (^^EC0.BAC1), 0xFFFF), One, Local4)
                    }
                    Else
                    {
                        Store (^^EC0.BAC1, Local4)
                    }

                    If (LEqual (^^EC0.BAM1, Zero))
                    {
                        Divide (Multiply (Local4, ^^EC0.BPV1), 0x03E8, , Local4)
                    }

                    Store (Local4, Index (PBST, One))
                    Store (Multiply (^^EC0.BRC1, BASC), Index (PBST, 0x02))
                    Store (^^EC0.BPV1, Index (PBST, 0x03))
                }
        ...
        Method (OSTP, 0, NotSerialized)
        {
            ...
            If (LEqual (OSYS, 0x07E0))
            {
                Store (0x06, Local1)
            }

            If (LEqual (OSYS, 0x07E1))
            {
                If (_OSI ("Windows 2017.2"))
                {
                    Store (0x07, Local1)
    ...
    Scope (_SB.PCI0.LPCB.EC0)
    {
        ...
        Device (VPC0)
        {
            ...
            Method (HALS, 0, NotSerialized)
            {
                ...
                If (LEqual (GGIV (0x02000012), Zero))
                {
                    And (0xFFFFFFBF, Local0, Local0)
                }
                Else
                {
            ...
            Method (GBID, 0, NotSerialized)
            {
                ...
                {
                    GBWD (0x16, 0x17)
                    Store (SMDW, Index (RETB, Zero))
                    ECSM (0x0B, 0x16, 0x37)
                    Store (SMD0, BFWB)
                    Store (FB70, Index (RETB, 0x02))
                }

                If (BOL1)
                {
                    GBWD (0x36, 0x17)
                    Store (SMDW, Index (RETB, One))
                    ECSM (0x0B, 0x36, 0x37)
                    Store (SMD0, BFWB)
                    Store (FB70, Index (RETB, 0x03))
                }
            ...
            Method (GSBI, 1, NotSerialized)
            {
                ...
                If (LEqual (Arg0, 0x02))
                {
                    CreateWordField (SBIF, Zero, BDCS)
                    Store (BDC1, BDCS)
                    CreateWordField (SBIF, 0x02, BFCS)
                    Store (BFC1, BFCS)
                    CreateWordField (SBIF, 0x04, BRCS)
                    Store (BRC1, BRCS)
                    CreateWordField (SBIF, 0x06, TTES)
                    Store (BRC1, TTES)
                    CreateWordField (SBIF, 0x08, TTFS)
                    GBWD (0x36, 0x13)
                    Store (SMDW, TTFS)
                    CreateWordField (SBIF, 0x0A, BPVS)
                    Store (BPV1, BPVS)
                    CreateWordField (SBIF, 0x0C, BPCS)
                    Store (BPC1, BPCS)
                    CreateWordField (SBIF, 0x0E, BTMS)
                    GBWD (0x36, 0x08)
                    Store (SMDW, BTMS)
                    CreateWordField (SBIF, 0x10, BMDS)
                    GBWD (0x36, 0x1B)
                    Store (SMDW, BMDS)
                    CreateWordField (SBIF, 0x12, FUDS)
                    GBWD (0x36, 0x3F)
                    Store (SMDW, FUDS)
                    CreateWordField (SBIF, 0x14, BDVS)
                    Store (BDV1, BDVS)
                }
